...
def main() -> None:
    ...
    cases = {
        f"linopy/{args.io_api}": functools.partial(
            run_linopy, data_dir, args.workdir / "model_linopy.lp", args.io_api
        ),
    ...
    for name, (res, wall) in results.items():
        print(
            f"{name:<24} {fmt_bytes(res.rss_bytes):>12} {fmt_bytes(res.peak_bytes):>12} {wall:>10.1f}s"
        )
