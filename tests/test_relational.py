...
def dispatch_eager_objective(gens: pd.DataFrame, load: pd.DataFrame) -> float:
    ...
    p = m.add_variables(
        lower=0, upper=p_max, coords=[li.index, gi.index], name="p", mask=mask
    )
...
def transport_data():
    ...
    pairs = [(buses[i], buses[(i + 1) % n_b]) for i in range(n_b)] + [
        (buses[0], buses[2])
    ]
    lines = pd.DataFrame(
        {
...
def test_nonlinear_product_rejected(dispatch_data):
    ...
    bad = Program(
        ...
        objective=ObjectiveDecl(
            "min", Sum(Var("p") * Var("p"), over=("generator", "snapshot"))
        ),
    )
    with DuckdbExecutor() as ex:
        with pytest.raises(RelationalBuildError, match="nonlinear"):
...
def test_missing_source_rejected(dispatch_data):
    ...
    with DuckdbExecutor() as ex:
        with pytest.raises(
            RelationalBuildError, match="no source bound for parameter 'cost'"
        ):
...
def test_out_of_foreach_dims_rejected(dispatch_data):
    ...
    with DuckdbExecutor() as ex:
        with pytest.raises(RelationalBuildError, match="missing a Sum"):
            ex.build(bad, dispatch_sources(gens, load))
