...
class TestValidateExpressions:
    def test_valid_schema_passes(self):
        schema = _schema(
            constraints={
                "cap": {"foreach": ["g"], "equations": [{"expression": "p <= p_max"}]}
            },
    ...
    def test_unknown_name_in_constraint(self):
        schema = _schema(
            constraints={
                "cap": {"foreach": ["g"], "equations": [{"expression": "q <= p_max"}]}
            },
        )
    ...
    def test_unknown_helper(self):
        schema = _schema(
            objectives={
                "cost": {"equations": [{"expression": "frobnicate(p, over=g)"}]}
            },
        )
...
class TestLoadTimeIntegration:
    ...
    def test_extend_sees_existing_model_variables(self, tmp_path):
        ...
        model = Model()
        model.add_variables(
            coords={"g": pd.Index(["wind", "solar"], name="g")}, name="p"
        )
