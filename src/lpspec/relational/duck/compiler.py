...
class DuckCompiler:
    ...
    def expression(self, expr: plan.Expression, context: str) -> CompiledExpression:
        ...
        if isinstance(expr, plan.Divide):
            return self._quotient(self.expression(expr.left, context), self.expression(expr.right, context), context)
    ...
    def _variable_fragment(self, name: str) -> TermFragment:
        ...
        rel = Rel(f'SELECT {cols}, var_label, 1.0 AS coeff FROM {table}', (*dims, 'var_label', 'coeff'))
        masked = self.program.variable(name).where is not None
        presence = Rel(f'SELECT {cols} FROM {table}', dims) if masked else None
        return TermFragment(
